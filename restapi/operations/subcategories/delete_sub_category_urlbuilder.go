...
func (o *DeleteSubCategoryURL) Build() (*url.URL, error) {
	...
	var _path = "/subcategories/{id}"

	id := swag.FormatInt64(o.ID)
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("ID is required on DeleteSubCategoryURL")
	}
...
func (o *DeleteSubCategoryURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on DeleteSubCategoryURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on DeleteSubCategoryURL")
	}
